...
(async function loud() {
  ...
  randomUser_API =
    "https://randomuser.me/api/?exc=info,registered,timezone,nat&results=5";
  ...
  $form.addEventListener("submit", async (event) => {
    ...
    try {
      const {
        ...
        `${BASE_API}list_movies.json?&query_term=${data.get("name")}`
      );
  ...
  } = await getData(`${BASE_API}movie_suggestions.json?movie_id=1`);
  ...
  function MyPlayListTemplate(data) {
    return `<li class="myPlaylist-item" data-id="${data.id}" data-category="${data.genres[0]}">
      <a href="#">
        <span>
          ${data.title}
        </span>
  ...
  function userTemplate(data) {
    return `<li class="playlistFriends-item" data-id="${data.login.uuid}">
              <a href="#">
  ...
  function userDescriptionTemplate(data) {
    return `<ul class="userDescriptionTemplate"> 
